...
def event_data():
    return {
        ...
        "address": "0x18994527E6FfE7e91F1873eCA53e900CE0D0f276",
...
def test_handle_event_deposit(mock_extract_event, event_data, db_session: Session):
    ...
    handle_event("0x18994527E6FfE7e91F1873eCA53e900CE0D0f276", event_data, "Deposit")
...
def test_handle_event_deposit_then_init_withdraw(mock_extract_event, event_data, db_session: Session):
    ...
    shares = 200_000000
    event_data['data'] = HexBytes("0x{:064x}".format(amount) + "{:064x}".format(shares))
    handle_event("0x18994527E6FfE7e91F1873eCA53e900CE0D0f276", event_data, "InitiateWithdraw")
    db_session.commit()
    user_portfolio = db_session.query(UserPortfolio).filter(
        UserPortfolio.user_address == "0x20f89ba1b0fc1e83f9aef0a134095cd63f7e8cc7"
    ).first()
    ...
    shares = 200_000000
    event_data['data'] = HexBytes("0x{:064x}".format(amount) + "{:064x}".format(shares))
    handle_event("0x18994527E6FfE7e91F1873eCA53e900CE0D0f276", event_data, "Withdrawn")
    db_session.commit()
    user_portfolio = db_session.query(UserPortfolio).filter(
        UserPortfolio.user_address == "0x20f89ba1b0fc1e83f9aef0a134095cd63f7e8cc7"
    ).first()
